<script lang="ts">
  ...
  const filteredIssues = $derived(
    (validation?.issues ?? result?.issues ?? []).filter(
...
<div class="page">
  ...
  <div class="page-header">
    ...
    <a class="btn-outline" href="/projects/{data.projectKey}/repository">← Back to Repository</a>
  </div>
  ...
  {#if step === 'upload'}
    <div class="wizard-layout">
      <div class="wizard-main glass">
        ...
        <h2 class="section-title" style="margin-top: 2rem;">Import Options</h2>

        <div class="options-grid">
          <div class="option-group">
            <label class="option-label" for="duplicate-strategy">Duplicate Strategy</label>
            <select id="duplicate-strategy" bind:value={duplicateStrategy} class="select">
              <option value="SKIP_EXISTING">Skip existing — ignore rows that already exist</option>
              <option value="UPDATE_EXISTING">Update existing — overwrite name, steps, and tags</option>
              <option value="FAIL_ON_DUPLICATE">Fail on duplicate — abort if any duplicate is found</option>
            </select>
            <p class="option-hint">How to handle a scenario that already exists in this project.</p>
          </div>

          <div class="option-group">
            <label class="option-label" for="default-status">Default Scenario Status</label>
            <select id="default-status" bind:value={defaultStatus} class="select">
              <option value="DRAFT">Draft — requires review before going live</option>
              <option value="ACTIVE">Active — publish immediately as live scenarios</option>
            </select>
            <p class="option-hint">Applied when the Excel row has no <code>status</code> column or the value is blank.</p>
          </div>
        </div>
      ...
      <div class="wizard-side">
        <div class="info-card glass">
          ...
          <p>Get the official Excel template with column definitions, examples, and allowed values.</p>
        ...
        <div class="info-card glass">
          ...
          <dl class="col-guide">
            ...
            <dd>Folder path separated by /.<br/><code>Checkout/Payment/VA</code></dd>
  ...
  {#if step === 'validated' && validation}
    <div class="wizard-layout">
      <div class="wizard-main">
        <div class="validation-summary glass">
          ...
          <div class="stats-row">
            ...
            <div class="stat accent">
              <span class="stat-value">{validation.newScenarioCount}</span>
              <span class="stat-label">New</span>
            </div>
            <div class="stat neutral">
              <span class="stat-value">{validation.existingScenarioCount}</span>
              <span class="stat-label">Existing</span>
            </div>
            <div class="stat">
              <span class="stat-value">{validation.parsedStepCount}</span>
              <span class="stat-label">Steps parsed</span>
            </div>
            <div class="stat {validation.errorCount > 0 ? 'danger' : 'success'}">
              <span class="stat-value">{validation.errorCount}</span>
              <span class="stat-label">Errors</span>
            </div>
            <div class="stat {validation.warningCount > 0 ? 'warning' : 'success'}">
          ...
          {#if validation.errorCount > 0}
            <div class="callout danger">
              <strong>Cannot import:</strong> {validation.errorCount} rows have errors that must be fixed before proceeding.
            </div>
          {:else if validation.warningCount > 0}
            <div class="callout warning">
              <strong>{validation.warningCount} warnings</strong> found. You can still import — review them below.
            </div>
          {:else}
            <div class="callout success">
              <strong>All rows look good!</strong> Ready to import {validation.newScenarioCount} new scenarios.
            </div>
          {/if}
        ...
        <div class="step-actions">
          ...
          <button
            ...
            disabled={validation.errorCount > 0 || busy}
          >
      ...
      <div class="wizard-side">
        <div class="info-card glass">
          ...
          <dl class="settings-summary">
            <dt>Duplicate strategy</dt>
            <dd>{duplicateStrategy.replace(/_/g, ' ')}</dd>
    ...
    <div class="wizard-layout">
      <div class="wizard-main">
        <div class="result-summary glass">
          <div class="result-icon {result.status === 'COMPLETED' ? 'success' : 'partial'}">
            {result.status === 'COMPLETED' ? '✓' : '⚠'}
          </div>
          <h2 class="section-title">
            {result.status === 'COMPLETED' ? 'Import complete' : 'Import finished with errors'}
          </h2>

          <div class="stats-row">
            <div class="stat accent">
              <span class="stat-value">{result.createdCount}</span>
              <span class="stat-label">Created</span>
            </div>
            <div class="stat neutral">
              <span class="stat-value">{result.updatedCount}</span>
            ...
            <div class="stat {result.failedCount > 0 ? 'danger' : 'success'}">
      ...
      <div class="wizard-side">
        <div class="info-card glass">
          <h3>Summary</h3>
          <dl class="settings-summary">
            <dt>Total rows</dt><dd>{result.totalRows}</dd>
            <dt>Created</dt><dd>{result.createdCount}</dd>
            <dt>Updated</dt><dd>{result.updatedCount}</dd>
            <dt>Skipped</dt><dd>{result.skippedCount}</dd>
            <dt>Failed</dt><dd>{result.failedCount}</dd>
          </dl>
        </div>
      </div>
    </div>
...
<style>
  ...
  .step.active .step-circle { background: var(--color-accent); color: #fff; }
  .step.active .step-label { color: var(--color-text); font-weight: 600; }
  .step.done .step-circle { background: var(--color-status-passed); color: #fff; }
  .step.done .step-label { color: var(--color-text-muted); }
  .step-circle { width: 1.75rem; height: 1.75rem; border-radius: 50%; border: 2px solid var(--color-border); display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 0.75rem; background: var(--color-surface); flex-shrink: 0; transition: background 0.2s; }
  ...
  /* Glass cards */
  .glass { background: var(--color-surface); border: 1px solid var(--color-border); border-radius: 12px; padding: 1.5rem; backdrop-filter: blur(8px); }
  ...
  .btn-primary { background: var(--color-accent); color: #fff; border: none; border-radius: 8px; padding: 0.6rem 1.5rem; font-size: 0.875rem; font-weight: 600; cursor: pointer; text-decoration: none; transition: background 0.15s; }
  .btn-primary:hover:not(:disabled) { background: var(--color-accent-hover); }
  .btn-primary:disabled { opacity: 0.5; cursor: not-allowed; }

  .btn-outline { background: transparent; color: var(--color-accent); border: 1px solid var(--color-accent); border-radius: 8px; padding: 0.55rem 1.25rem; font-size: 0.875rem; font-weight: 500; cursor: pointer; text-decoration: none; transition: background 0.15s; display: inline-block; }
  ...
  .segmented button { background: none; border: none; border-right: 1px solid var(--color-border); color: var(--color-text-muted); padding: 0.3rem 0.75rem; font-size: 0.75rem; cursor: pointer; }
  ...
  /* Validation summary */
  .validation-summary .file-badge { font-size: 0.8rem; color: var(--color-text-muted); margin: 0 0 1rem; }
  ...
  /* Importing state */
  .importing-state { display: flex; flex-direction: column; align-items: center; gap: 1rem; text-align: center; padding: 4rem 2rem; max-width: 480px; margin: 0 auto; }
