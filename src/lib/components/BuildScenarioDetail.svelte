<script lang="ts">
  ...
  import { getRunResult, type ScenarioRunResult, type StepRunResult } from '$lib/api/runs';
  import type { BuildScenario } from '$lib/api/builds';
  ...
  interface Props {
    scenario: BuildScenario | null;
    projectKey: string;
    onclose: () => void;
    onupdateresult?: (scenario: BuildScenario) => void;
  }

  let { scenario, projectKey, onclose, onupdateresult }: Props = $props();

  let scenarioDetail = $state<Scenario | null>(null);
  let runResult = $state<ScenarioRunResult | null>(null);
  ...
  $effect(() => {
    const s = scenario;
    if (!s) { scenarioDetail = null; runResult = null; return; }
    loading = true;
    scenarioDetail = null;
    runResult = null;
    const tasks: Promise<unknown>[] = [
      getScenario(projectKey, s.scenarioId)
        .then(d => { scenarioDetail = d; })
        .catch(() => {})
    ];
    if (s.sourceType === 'AUTOMATION' && s.sourceId) {
      tasks.push(
        getRunResult(projectKey, s.sourceId)
          .then(r => { runResult = r; })
          .catch(() => {})
      );
    }
    Promise.all(tasks).finally(() => { loading = false; });
  });
  ...
  function statusVariant(s: string): 'success' | 'danger' | 'info' | 'warning' | 'neutral' {
    switch (s?.toUpperCase()) {
      ...
      case 'NOT_EXECUTED': return 'neutral';
...
{#if scenario}
  ...
  <div class="panel" role="dialog" aria-label="Scenario detail" aria-modal="true">

    <div class="panel-header">
      <div class="panel-header-left">
        <Badge text={scenario.latestStatus} variant={statusVariant(scenario.latestStatus)} />
        <span class="panel-title">{scenario.name}</span>
      </div>
    ...
    <div class="panel-body">
      ...
      <div class="meta-grid">
        <div class="meta-item">
          ...
          <span class="meta-value mono small">{scenario.scenarioKey}</span>
        </div>
        {#if scenario.priority}
          <div class="meta-item">
            <span class="meta-label">Priority</span>
            <span class="meta-value">{scenario.priority}</span>
          </div>
        {/if}
        {#if scenario.featureName}
          <div class="meta-item">
            <span class="meta-label">Feature</span>
            <span class="meta-value">{scenario.featureName}</span>
          </div>
        {/if}
        {#if scenario.executedBy}
          <div class="meta-item">
            <span class="meta-label">Executed By</span>
            <span class="meta-value">{scenario.executedBy}</span>
          </div>
        {/if}
        {#if scenario.executedAt}
          <div class="meta-item">
            <span class="meta-label">Executed At</span>
            <span class="meta-value">{fmt(scenario.executedAt)}</span>
          </div>
        {/if}
        {#if scenario.durationMs != null}
          <div class="meta-item">
            <span class="meta-label">Duration</span>
            <span class="meta-value">{durationMs(scenario.durationMs)}</span>
          </div>
        {/if}
      </div>
      ...
      {#if scenario.notes}
        <div class="notes-block">
          ...
          <p class="notes-text">{scenario.notes}</p>
        </div>
      {/if}
      ...
      {#if scenario.source !== 'AUTOMATION' && onupdateresult}
        <div class="action-row">
          <button class="action-btn" onclick={() => { onclose(); onupdateresult?.(scenario!); }}>
      ...
      <div class="steps-section">
        <h3 class="steps-title">
          Steps
          {#if scenarioDetail}
            <span class="steps-count">{scenarioDetail.steps.length}</span>
          {/if}
        </h3>

        {#if loading}
          <div class="steps-loading">
            <SetaraLoader mode="progress" size={32} label="Loading" />
                ...
                <div class="step-card-body">
                  ...
                  {#if step.description}
                    <div class="step-section">
                      ...
                      <MarkdownBlock value={step.description} collapsedHeight={100} />
                    </div>
                  {/if}
                </div>
              </li>
      ...
      <div class="footer-links">
        <a href="/projects/{projectKey}/repository?scenario={scenario.scenarioId}" class="footer-link">
...
<style>
  ...
  :global([data-theme="dark"]) .overlay { background: rgba(0,0,0,0.5); }
  ...
  .action-btn:hover { background: color-mix(in srgb, var(--color-accent), black 12%); }
  ...
  .steps-empty { font-size: 0.8rem; color: var(--color-text-muted); font-style: italic; }
  ...
  .step-card--passed { border-color: color-mix(in srgb, var(--color-success, #0d9488), transparent 65%); }
  ...
  .step-num { font-size: 0.72rem; font-weight: 800; color: var(--color-text-muted); min-width: 20px; font-family: ui-monospace, monospace; }
  ...
  .step-text { font-size: 0.9rem; font-weight: 600; color: var(--color-text); line-height: 1.4; }
  ...
  .footer-links { padding-top: 16px; border-top: 1px solid var(--color-border); }
  ...
  .footer-link:hover { background: color-mix(in srgb, var(--color-accent), black 12%); }
</style>
